...
class AddModal extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      color:'',
      projectId:this.props.storyType,
      loading:false,
      users:[]
    };
  ...
  static getDerivedStateFromProps(props, state) {
    return {projectId: props.storyType}
  }

handleContributor(e) {
  const {users} = this.state;
  const contributor = users.filter(user => user.firstName + " " + user.lastName === e.target.value).slice()
  this.setState({[e.target.name]: contributor})
}
    ...
    return (
      <div>
        ...
        <FormGroup>
            ...
            <Input type="select" name="contributors" id="contributors" onChange={this.handleContributor.bind(this)}>
